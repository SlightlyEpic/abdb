...
pub async fn init_heap_file<B: BufferPool>(bp: &B, file_id: FileId, table_oid: OId) -> Result<()> {
    ...
    buffer.fill(0);
    let temp_buffer =
        HeapFileHeaderPage::init([0u8; constants::PAGE_BUF_SIZE], header_lpage_id, table_oid);
...
pub async fn bootstrap_database<B, D, A>(
    ...
    clog: std::sync::Arc<crate::clog::CommitLog>,
) -> Result<()>
where
    B: BufferPool,
    D: PageDirectory,
    A: PageAllocator,
{
    ...
    let temp_tm =
        std::sync::Arc::new(crate::transaction::TransactionManager::new().with_clog(clog));
    ...
    A: PageAllocator,
{
    ...
    let temp_tm = std::sync::Arc::new(
        crate::transaction::TransactionManager::with_next_txn_id(u64::MAX).with_clog(clog),
    );
    let mut txn = temp_tm.begin(crate::common::txn::IsolationLevel::Snapshot);
    txn.read_ts = u64::MAX;
    ...
    while let Some(result) = stream.next().await {
        let (tuple_bytes, _rid) =
            result.map_err(|e| DbError::Internal(format!("error reading sys_tables: {:?}", e)))?;
    ...
    while let Some(result) = stream.next().await {
        let (tuple_bytes, _rid) =
            result.map_err(|e| DbError::Internal(format!("error reading sys_columns: {:?}", e)))?;
        let type_id_raw = columns_layout
            .read_field("type_id", 3, &tuple_bytes)
            .and_then(|v| v.as_u8())
            .unwrap_or(0);
        let type_id = DataType::from_u8(type_id_raw);
        let default_val_str = columns_layout
            .read_field("default_val", 8, &tuple_bytes)
            .and_then(|v| v.as_string());
    ...
    while let Some(result) = stream.next().await {
        let (tuple_bytes, _rid) =
            result.map_err(|e| DbError::Internal(format!("error reading sys_indexes: {:?}", e)))?;

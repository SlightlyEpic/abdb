...
pub enum BindError {
    ...
    TypeMismatch { expected: DataType, found: DataType, context: String },
    Unsupported(String),
    CatalogError(String),
...
impl fmt::Display for BindError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::SubqueryTooManyColumns   => write!(f, "scalar subquery must return exactly one column"),
            Self::MultiColumnIndexUnsupported => write!(f, "multi-column indexes are not yet supported"),
            ...
            Self::ColumnCountMismatch { expected, found } =>
                write!(f, "column count mismatch: expected {expected}, found {found}"),

//! B-Tree Index Leaf Page Overlay
//!
//! Terminal nodes of the index B-Tree that store mappings from index keys
//! to heap tuple locations (RecordId = page + slot).
//!
...
//! 16-39   | 24   | leaf_header  | BTreeLeafHeader
//! 40-4095 | 4056 | entries[]    | BTreeLeafEntry array
//!
//! # Thread Safety
//!
...
/// Size of each BTreeLeafEntry in bytes.
const ENTRY_SIZE: usize = size_of::<BTreeLeafEntry>();

/// Offset where entries begin (after UberPageHeader + LeafHeader).
const ENTRIES_OFFSET: usize = UBER_HEADER_SIZE + size_of::<BTreeLeafHeader>();
...
/// Type-specific header for B-Tree leaf pages.
///
...
const _: () = assert!(size_of::<BTreeLeafHeader>() == 24);

// ============================================================================
// ENTRY STRUCTURE
// ============================================================================

/// A single entry in a B-Tree leaf page.
///
/// Maps an index key to a heap tuple location (RecordId).
///
/// # Layout (16 bytes, C repr)
///
/// ```text
/// Bytes 0-7:   key          (u64) - Index key value
/// Bytes 8-11:  record_page  (u32) - Heap page containing the tuple
/// Bytes 12-13: record_slot  (u16) - Slot within the heap page
...
const _: () = assert!(size_of::<BTreeLeafEntry>() == 16);

impl BTreeLeafEntry {
    #[inline]
    pub fn new(key: u64, record_page: LPageId, record_slot: SlotId) -> Self {
        Self {
            key,
            record_page,
            record_slot,
    ...
    #[inline]
    pub fn record_id(&self) -> (LPageId, SlotId) {
...
where
    T: AsRef<[u8]>,
{
    /// Wrap a buffer as a B-Tree leaf page.
    ///
    /// # Panics
    ///
    ...
    pub fn new(data: T) -> Self {
        ...
        assert!(
            len == PAGE_BUF_SIZE,
            "BTreeLeafPage::new called with buffer of size {} (expected {})",
            len,
            PAGE_BUF_SIZE
        );
        Self { data }
    ...
    /// Wrap a buffer and validate it's a B-Tree leaf page.
    ///
    /// # Errors
    ///
    /// Returns `OverlayError::TypeMismatch` if the page type is not BTreeLeaf.
    ...
    #[inline]
    fn leaf_header(&self) -> BTreeLeafHeader {
        BTreeLeafHeader::read_from_prefix(&self.data.as_ref()[UBER_HEADER_SIZE..])
            .expect("BTreeLeafHeader must fit after UberPageHeader")
            .0
    }
    ...
    pub fn entry(&self, index: u16) -> Result<BTreeLeafEntry, OverlayError> {
        ...
        Ok(
            BTreeLeafEntry::read_from_prefix(&self.data.as_ref()[offset..])
    ...
    pub fn find_slot(&self, target_key: u64) -> (bool, u16) {
        ...
        while left < right {
            ...
            let mid_key = self.entry(mid).unwrap().key;
    ...
    pub fn lookup(&self, key: u64) -> Option<(LPageId, SlotId)> {
        ...
        if found {
            Some(self.entry(pos).unwrap().record_id())
        } else {
            None
        }
    }
    ...
    T: AsRef<[u8]> + AsMut<[u8]>,
{
    ...
    pub fn init(mut data: T, page_id: LPageId) -> Self {
        ...
        uber.page_type_id = PageType::BTreeLeaf as u8;
    ...
    fn set_entry(&mut self, index: u16, entry: BTreeLeafEntry) {
        assert!(
            index < MAX_ENTRIES,
    ...
    pub fn insert_entry(
        ...
        let (found, insert_pos) = self.find_slot(key);
        if found {
            return Err(OverlayError::DuplicateKey { key });
        }
        ...
        self.set_entry(
            insert_pos,
            BTreeLeafEntry::new(key, record_page, record_slot),
        );
    ...
    /// Update the record location for an existing key.
    ...
    pub fn update_entry(
        ...
        let (found, pos) = self.find_slot(key);
        if found {
            self.set_entry(pos, BTreeLeafEntry::new(key, new_page, new_slot));
            Ok(true)
        } else {
            Ok(false)
        }
...
mod tests {
    ...
    fn test_leaf_header_offsets() {
        ...
        let mut page = BTreeLeafPage::init(buffer, 42);
    ...
    fn test_entry_structure() {
        let entry = BTreeLeafEntry::new(100, 5, 3);
        assert_eq!(entry.key, 100);
        assert_eq!(entry.record_page, 5);
        assert_eq!(entry.record_slot, 3);
        assert_eq!(entry.record_id(), (5, 3));
    }

    #[test]
    fn test_init() {
        let buffer = [0u8; PAGE_BUF_SIZE];
        let page = BTreeLeafPage::init(buffer, 42);

        assert_eq!(page.page_id(), 42);
        assert_eq!(page.page_type_byte(), PageType::BTreeLeaf as u8);
    ...
    fn test_binary_search() {
        let buffer = [0u8; PAGE_BUF_SIZE];
        let mut page = BTreeLeafPage::init(buffer, 1);
        ...
        assert_eq!(page.entry(0).unwrap().key, 100);
        assert_eq!(page.entry(1).unwrap().key, 200);
        assert_eq!(page.entry(2).unwrap().key, 300);
        assert_eq!(page.entry(3).unwrap().key, 400);
        assert_eq!(page.entry(4).unwrap().key, 500);
    ...
    fn test_delete_entry() {
        ...
        assert_eq!(page.entry(0).unwrap().key, 100);
        assert_eq!(page.entry(1).unwrap().key, 300);
    }
    ...
    fn test_delete_by_key() {
        ...
        assert_eq!(page.entry(0).unwrap().key, 200);
    }
    ...
    fn test_from_buffer_validation() {
        let buffer = [0u8; PAGE_BUF_SIZE];
        let page = BTreeLeafPage::init(buffer, 1);

        let valid_buffer: [u8; PAGE_BUF_SIZE] = page.as_buffer().try_into().unwrap();
        let result = BTreeLeafPage::from_buffer(valid_buffer);
        assert!(result.is_ok());

        let mut bad_buffer = [0u8; PAGE_BUF_SIZE];
        bad_buffer[12] = PageType::BTreeInner as u8;
        let result = BTreeLeafPage::from_buffer(bad_buffer);

//! B-Tree Index Inner Page Overlay
//!
//! Routing layer for the index B-Tree that guides searches to appropriate
...
//! 16-23   | 8    | inner_header   | BTreeInnerHeader
//! 24-4095 | 4072 | entries[]      | BTreeInnerEntry array
//! ```
//!
...
/// Size of each BTreeInnerEntry in bytes.
const ENTRY_SIZE: usize = size_of::<BTreeInnerEntry>();

/// Offset where entries begin (after UberPageHeader + InnerHeader).
const ENTRIES_OFFSET: usize = UBER_HEADER_SIZE + size_of::<BTreeInnerHeader>();
...
/// Type-specific header for B-Tree inner pages.
///
...
/// A single entry in a B-Tree inner page.
///
...
pub struct BTreeInnerEntry {
    pub separator_key: u64,
    pub right_child: LPageId,
    _padding: u32,
}

const _: () = assert!(size_of::<BTreeInnerEntry>() == 16);

impl BTreeInnerEntry {
    #[inline]
    ...
    T: AsRef<[u8]>,
{
    ...
    pub fn new(data: T) -> Self {
        ...
        assert!(
            len == PAGE_BUF_SIZE,
            "BTreeInnerPage::new called with buffer of size {} (expected {})",
            len,
            PAGE_BUF_SIZE
        );
        Self { data }
    ...
    fn validate_type(&self) -> Result<(), OverlayError> {
        ...
        if page_type != PageType::BTreeInner {
            return Err(OverlayError::TypeMismatch {
                expected: PageType::BTreeInner,
                found: page_type,
            });
        }
        Ok(())
    ...
    #[inline]
    fn inner_header(&self) -> BTreeInnerHeader {
        BTreeInnerHeader::read_from_prefix(&self.data.as_ref()[UBER_HEADER_SIZE..])
            .expect("BTreeInnerHeader must fit after UberPageHeader")
            .0
    }
    ...
    pub fn entry(&self, index: u16) -> Result<BTreeInnerEntry, OverlayError> {
        ...
        Ok(
            BTreeInnerEntry::read_from_prefix(&self.data.as_ref()[offset..])
...
where
    T: AsRef<[u8]> + AsMut<[u8]>,
{
    /// Initialize a new B-Tree inner page.
    pub fn init(mut data: T, page_id: LPageId, leftmost_child: LPageId) -> Self {
        ...
        uber.page_type_id = PageType::BTreeInner as u8;
        uber.write_to_prefix(buffer)
            .expect("UberPageHeader must fit in page buffer");

        let inner = BTreeInnerHeader {
            num_keys: 0,
            flags: 0,
            leftmost_child,
        };
        inner
            ...
            .expect("BTreeInnerHeader must fit");
    ...
    #[inline]
    fn update_inner_header(&mut self, f: impl FnOnce(&mut BTreeInnerHeader)) {
        let mut header =
            BTreeInnerHeader::read_from_prefix(&self.data.as_ref()[UBER_HEADER_SIZE..])
                .expect("BTreeInnerHeader must fit")
                .0;
    ...
    fn set_entry(&mut self, index: u16, entry: BTreeInnerEntry) {
        assert!(
            index < MAX_KEYS,
    ...
    pub fn insert_separator(
        ...
        self.set_entry(insert_pos, BTreeInnerEntry::new(separator_key, right_child));
...
mod tests {
    ...
    fn test_entry_structure() {
        let entry = BTreeInnerEntry::new(100, 5);
    ...
    fn test_init() {
        ...
        let page = BTreeInnerPage::init(buffer, 42, 99);

        assert_eq!(page.page_id(), 42);
        assert_eq!(page.page_type_byte(), PageType::BTreeInner as u8);
    ...
    fn test_routing_empty_node() {
        let buffer = [0u8; PAGE_BUF_SIZE];
        let page = BTreeInnerPage::init(buffer, 1, 99);
    ...
    fn test_routing_single_separator() {
        let buffer = [0u8; PAGE_BUF_SIZE];
        let mut page = BTreeInnerPage::init(buffer, 1, 10);
    ...
    fn test_insertion_order() {
        let buffer = [0u8; PAGE_BUF_SIZE];
        let mut page = BTreeInnerPage::init(buffer, 1, 0);
    ...
    fn test_from_buffer_validation() {
        let buffer = [0u8; PAGE_BUF_SIZE];
        let page = BTreeInnerPage::init(buffer, 1, 0);

        let valid_buffer: [u8; PAGE_BUF_SIZE] = page.as_buffer().try_into().unwrap();
        let result = BTreeInnerPage::from_buffer(valid_buffer);
        assert!(result.is_ok());

        let mut bad_buffer = [0u8; PAGE_BUF_SIZE];
        bad_buffer[12] = PageType::BTreeLeaf as u8;
        let result = BTreeInnerPage::from_buffer(bad_buffer);
